...
- Enh: Custom side menu via `app\widgets\admin\Module::items` (mdmunir).
...
- Chg: Using `VarDumper::export` to save `data` of `app\widgets\models\AuthItem` (mdmunir).
...
- Chg: Can customize return of `app\widgets\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `app\widgets\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `app\widgets\admin\Module::allowActions`. Set access control directly with `app\widgets\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `app\widgets\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
